//
//  AuraServiceFetchAccountDetailsMock.swift
//  AuraTests
//
...
final class AuraServiceFetchAccountDetailsMock {
	
	//MARK: -Properties
	var isLoading: Bool = false
	var response: Int = 0
	var currentBalance: Decimal
	let auraServiceResponseMock: AccountResponse
	...
	init() {
		...
		auraServiceResponseMock = AccountResponse(transactions: [transaction1, transaction2, transaction3, transaction4, transaction5], currentBalance: currentBalance)
		
	}
	
	//MARK: -Methods
	private func encodeData(AuraServiceResponseTypeMock :  AccountResponse) throws -> Data { //encode en Data : binaire
		return try JSONEncoder().encode(AuraServiceResponseTypeMock)
	}
	...
	func validMockResponse(request: URLRequest) async throws -> (Data, URLResponse) {
		let data = try encodeData(AuraServiceResponseTypeMock : auraServiceResponseMock)
	...
	private func invalidMockResponseRequestFailedError(request: URLRequest) async throws -> (Data, URLResponse) {
		let invalidData = try encodeData(AuraServiceResponseTypeMock : auraServiceResponseMock)
